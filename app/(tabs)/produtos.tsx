...
export default function ProdutosScreen() {
  ...
  const renderGridItem = ({ item }: { item: Produto }) => {
    const status = getStatusBadge(item);
    return (
      <View className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl p-4 m-2 max-w-[46%]">
        <View className="h-12 w-12 items-center justify-center rounded-xl bg-zinc-800 mb-3">
          <Package size={22} color={THEME.dark.mutedForeground} />
        </View>
